...
describe('> Transaction controller [transfer]', () => {
    it('should create a transfer successfully', async () => {
        ...
        // Act
        const res = mockRes()
        await Controller.transfer(mockReq, res)

        // Assert
        expect(logger).toHaveBeenCalledWith('\n> [SUCCESS] Created')
        expect(res.status).toHaveBeenCalledWith(201)
        expect(res.json).toHaveBeenCalledWith({ code: 201, status: true, message: 'Transfer created successfully' })
    })
    ...
    it('should return an error of not enough money', async () => {
        ...
        expect(res.status).toHaveBeenCalledWith(200)
        expect(res.json).toHaveBeenCalledWith({ code: 200, status: false, message: 'Not enough money to complete this transfer' })
    })
